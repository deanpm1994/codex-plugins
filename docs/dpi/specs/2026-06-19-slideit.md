...
- Ingest a ChatGPT or search transcript and turn the whole context into a presentation.
...
- Keep the rendering layer swappable later even though v1 standardizes on Reveal.js.
- Add the plugin to the personal marketplace when the plugin is ready.
- Adaptively show, hide, or ask for content when the source transcript does not make disclosure obvious.
...
6. Publish the plugin entry in the personal marketplace once the plugin is validated.
7. Offer to create a deck proactively when a transcript or search session looks presentation-worthy.
8. Decide when to surface, hide, or request source content based on the intended audience and the transcript.
...
The plugin lives as a standalone Codex plugin under `plugins/slideit`. It should be self-contained and not depend on any other plugin's internals.
...
- a loose outline
- existing notes
- a ChatGPT or search transcript
...
7. Add or update the plugin's marketplace entry once the implementation is complete.
...
When the source is a transcript, the plugin should keep enough traceability to show what came from the original context, even if the final deck is a cleaned-up synthesis rather than a literal export. If disclosure is ambiguous, the plugin should ask the user before deciding what to hide or expose.
...
- it can synthesize a deck from a ChatGPT or search transcript
- the deck opens with a polished visual design
- diagram slides can use Excalidraw-generated assets
- it can show, hide, or request source content when transcript disclosure is unclear
...
- the plugin is listed in the personal marketplace
...
Write the implementation plan for the plugin scaffold, manifest, skills, deck-generation flow, and marketplace entry.

### Editable Notes

The key design constraint is design quality first. The generated deck should look intentional and polished even when the input is rough. The marketplace update should happen at the end of the implementation, not as a separate side task.
